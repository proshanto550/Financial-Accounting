...
const BalanceSheet = ({ accounts, balances, getTotalAssets, getTotalLiabilities, getTotalEquity, incomeStatement, t }) => {
    ...
    return (
        <div className={`rounded-xl ${t.cardBg} ${t.shadow} p-6 md:p-10 ${t.border} max-w-full mx-auto`}>
            <div className="flex justify-between items-center mb-6">
                <h2 className={`text-4xl font-bold ${t.secondaryAccent} text-center`}>BALANCE SHEET</h2>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-5">
                <div>
                    <h3 className={`text-3xl font-semibold ${t.accent} mb-3 border-b-2 border-cyan-700/50 pb-2`}>ASSETS</h3>
                    <div className="space-y-1">
                        {assetAccounts.map(acc => (
                            <div key={acc.id} className="flex justify-between text-xl py-1">
                ...
                <div>
                    <h3 className={`text-3xl font-semibold ${t.secondaryAccent} mb-3 border-b-2 border-lime-700/50 pb-2`}>LIABILITIES & EQUITY</h3>

                    {/* Liabilities */}
                    <h4 className={`text-2xl font-semibold  ${t.accent} mb-3`}>Liabilities</h4>
                    <div className="space-y-1">
                    ...
                    {/* Equity */}
                    <h4 className={`text-2xl font-semibold my-4 pt-5 border-t ${t.accent} border-neutral-700`}>Equity</h4>
                    <div className="space-y-1">
            ...
            <div className={`mt-4 p-4 rounded-lg text-xl ${Math.abs(totalAssets - totalEquityAndLiabilities) < 0.01 ? `bg-lime-900/30 ${t.accent}` : 'bg-red-900/30 text-red-400'}`}>

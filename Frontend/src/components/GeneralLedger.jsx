    ...
    return (
        <div className={`rounded-xl ${t.cardBg} ${t.shadow} p-6 ${t.border}`}>
            <div className="flex justify-between items-center mb-6">
                <h2 className={`text-2xl font-bold ${t.secondaryAccent}`}>GENERAL LEDGER</h2>
                <button
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-neutral-700">
                                    <thead className={`${t.tableHeader}`}>
                                        <tr>
                                            <th className="px-6 py-4 text-left text-base font-mono uppercase">Date</th>
                                            ...
                                            <th className="px-6 py-4 text-right text-base font-mono uppercase">Credit</th>
                                        </tr>
                                    </thead>
                                    <tbody className={`${t.cardBg} divide-y divide-neutral-700/50`}>
                                        {relevantEntries.map((line, index) => (
                                            <tr key={index} className={t.tableRow}>
                                                <td className={`px-6 py-3 whitespace-nowrap text-base text-slate-400`}>{line.date}</td>
                                                <td className={`px-6 py-3 whitespace-nowrap text-base ${t.text}`}>{line.description || '-'}</td>
                                                <td className={`px-6 py-3 whitespace-nowrap text-base ${t.text} text-right font-mono`}>{parseFloat(line.debit || 0).toFixed(2)}</td>
                                                <td className={`px-6 py-3 whitespace-nowrap text-base ${t.text} text-right font-mono`}>{parseFloat(line.credit || 0).toFixed(2)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot className={`font-bold ${t.tableHeader}`}>
                                        <tr>
                                            <td colSpan="2" className={`px-6 py-3 text-right text-base ${t.text} font-mono`}>Total Balance:</td>
                                            <td colSpan="2" className={`px-6 py-3 text-right text-base ${t.secondaryAccent} font-mono`}>

...
const IncomeStatement = ({ accounts, balances, incomeStatement, t }) => {
    ...
    return (
        <div className={`rounded-xl ${t.cardBg} ${t.shadow} p-6 md:p-8 ${t.border} w-full mx-auto`}>
            <div className={`overflow-x-auto md:border md:p-10 md:pl-20 md:pr-20 rounded-xl ${t.border}`}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className={`text-3xl font-bold ${t.secondaryAccent} text-center`}>INCOME STATEMENT</h2>
                    <button
                ...
                <div className="space-y-6">
                    {/* Revenue Section */}
                    <div>
                        <h3 className={`text-xl font-bold ${t.accent} mb-3 border-b border-neutral-700 pb-1`}>REVENUE</h3>
                        {revenueAccounts.length === 0 ? <p className="text-slate-500 text-base italic">No revenue recorded</p> : (
                        ...
                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-dashed border-neutral-700">
                            <span className={`font-semibold text-lg ${t.text}`}>Total Revenue</span>
                            <span className={`font-mono text-xl text-lime-400`}>${revenue.toFixed(2)}</span>
                        </div>
                    </div>

                    {/* Expenses Section */}
                    <div>
                        <h3 className={`text-xl font-bold ${t.accent} mb-3 border-b border-neutral-700 pb-1`}>EXPENSES</h3>
                        {expenseAccounts.length === 0 ? <p className="text-slate-500 text-base italic">No expenses recorded</p> : (
                            <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-dashed border-neutral-700">
                            <span className={`font-semibold text-lg ${t.text}`}>Total Expenses</span>
                    ...
                    <div className={`flex justify-between items-center pt-5 border-t-2 ${t.border}`}>
                        ...
                        <span className={`text-2xl font-bold font-mono ${netIncome >= 0 ? 'text-lime-400' : 'text-red-500'}`}>${netIncome.toFixed(2)}</span>
                    </div>
                </div>
            </div>
        </div>

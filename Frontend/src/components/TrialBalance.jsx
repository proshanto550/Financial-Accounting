    ...
    return (
        <div className={`rounded-xl ${t.cardBg} ${t.shadow} p-6 ${t.border}`}>
            <div className="flex justify-between items-center mb-6">
                <h2 className={`text-2xl font-bold ${t.secondaryAccent}`}>TRIAL BALANCE</h2>
                <button
            ...
            <div className={`overflow-x-auto border ${t.border} rounded-lg`}>
                <table className="min-w-full divide-y divide-neutral-700">
                    <thead className={t.tableHeader}>
                        <tr>
                            <th className="px-6 py-4 text-left text-base font-mono uppercase">Account Name</th>
                            <th className="px-6 py-4 text-center text-base font-mono uppercase">Debit</th>
                    ...
                    <tbody className={`${t.cardBg} divide-y divide-neutral-700/50`}>
                        {data.map((item, index) => (
                            <tr key={item.account?.id || index} className={t.tableRow}>
                                <td className={`px-6 py-3 text-base ${t.text}`}>{item.account?.name}</td>
                                <td className={`px-6 py-3 whitespace-nowrap text-base ${t.text} text-center font-mono text-lime-400`}>${(item.debit || 0).toFixed(2)}</td>
                                <td className={`px-6 pr-6 py-3 whitespace-nowrap text-base ${t.text} text-right font-mono text-cyan-400`}>${(item.credit || 0).toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot className={`font-bold ${t.tableHeader}`}>
                        <tr>
                            <td colSpan="1" className={`px-6 py-4 text-right text-base ${t.text} font-mono`}>TOTALS:</td>
                            <td className={`px-6 py-4 text-center text-base ${t.secondaryAccent} font-mono`}>${totalDebits.toFixed(2)}</td>
                            <td className={`px-6 pr-6 py-4 text-right text-base ${t.secondaryAccent} font-mono`}>${totalCredits.toFixed(2)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
